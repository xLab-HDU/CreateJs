...
(lib.阴影 = function() {
	this.initialize(ss["最后防线_atlas_1"]);
	this.gotoAndStop(0);
...
// stage content:
(lib.最后防线 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	...
	this.frame_0 = function() {
		...
		var endMenuContainer = new cjs.Container();//结束界面管理容器
		//计时器
		var sunTimer = null;
		...
		var isDragging = false;//标志玩家是否在使用移除功能;@布尔变量统一加前缀is✅
		...
		function removeActor(){
			...
			if (actorRow >= 0 && actorCol >= 0 && actorRow < 3 && actorCol < 9 && actorsArray[actorRow][actorCol] == 1){
				var removedActor = actorContainer.getChildByName("actor_"+actorRow+"_"+actorCol);
				actorContainer.removeChild(removedActor);
		...
		const ZombieConfig = {
			...
			punkZombie:{
				...
				deadClassName: "deadPunkZombieMC", 
				runningClassName: "runningPunkZombieMC",//朋克僵尸奔跑精灵的链接名
				damage: 10,
				blood: 150,
				speed: 0.5,
				originalSpeed: 0.5,
		...
		function createZombie(zombieType) {
			...
			newZombie.moveOrAttack = function() {
				if (this.isDying) return;// 如果僵尸已经死亡，不再执行攻击和移动
			...
			zombieContainer.addChild(newZombie);
			zombiesArray[newZombie.zombieRow].push(newZombie.name);
		}
		...
		function addActorsCard(){
			//射手卡片
			createActorCard({
				movingClassName: "ArcherMovingMC",// 移动的射手
				actorName: "ArcherMC",      // 部署的射手
				attackName:"ArcherAttackMC",// 攻击状态的射手
				num: 0,
				cost: 100,
				fireRate: 30,//开火频率
				recharge: 30,//填充进度
		...
		function onActorClicked(actorCard){
			...
			if (money>=cost && !isMovingActor) {
				...
				movingActor.addEventListener("click",placeActor);
				overlayContainer.addChild(movingActor);
		...
		function placeActor(evt){
			...
			if (actorRow>=0&&actorCol>=0&&actorRow<3&&actorCol<9&&actorsArray[actorRow][actorCol]==0) {
				money -= actorCard.cost;// 扣除金币
				updateMoney();
				//创建角色
				createActor(actorCard,actorRow,actorCol);
				isMovingActor=false;
		...
		function createActor(actorCard,actorRow,actorCol) {
		    ...
		    // 角色容器
		    const spriteContainer = new cjs.Container();//角色精灵容器
			...
			placedActor.fire = function () {//@用统一调度器的方式，做个实验测试内存占用差距
				...
				//①“非攻击状态”，除非出现敌人，否则始终保持站立
				if (!this.isAttacking) {   
					if (this.recharge >= this.fireRate && hasEnemyAhead) {  
						this.playAttack();  
				...
				//③发射子弹，前提是“攻击状态+有敌人”（就是避开上面两个条件）
				if (this.attackSprite.currentFrame >= this.attackSprite.totalFrames - 1) {  
					createBullet(this);  
					this.playIdle();  
					this.isAttacking = false;  
					this.recharge = 0;  
			...
			updateActorMoving: function() {
				if (isMovingActor) {
					...
					if (actorRow>=0&&actorCol>=0&&actorRow<3&&actorCol<9&&actorsArray[actorRow][actorCol] == 0) {
						...
						selector.x=borderLeft+actorCol*gridWidth;
...
lib.properties = {
	...
	opacity: 1.00,
	manifest: [
		{src:"images/最后防线_atlas_1.png?1754117047067", id:"最后防线_atlas_1"}
	],
	preloads: []
};
